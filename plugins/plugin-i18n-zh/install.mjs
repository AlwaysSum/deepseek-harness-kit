// Bootstrap installer for @dsh-kit/plugin-i18n-zh into a dsh profile.
//
// Usage:
...
// under plugins/plugin-i18n-zh apply on the next dsh restart) and records it in
// the profile manifest (dependencies + dsh.profile.bundles), which is what makes
// the bundle's cordis.patch.yml mount at boot. Pass --uninstall to remove both.
...
const NAME = "@dsh-kit/plugin-i18n-zh";
...
const linkPath = join(linkDir, "plugin-i18n-zh");
...
console.log("重新启动 dsh 服务后，汉化 UI 生效。");
