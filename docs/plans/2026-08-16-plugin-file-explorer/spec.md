...
- FR3.7 **按文件类型分派编辑组件**：文件 Tab 内容窗口按扩展名呈现不同组件——txt/其他代码 → 行号 + CodeMirror 高亮编辑；json → 专门 JSON 编辑器（实时语法校验、格式化/压缩按钮、非法 JSON 阻止保存）；md → 左编辑右预览分屏（内置轻量 Markdown 渲染，标题/列表/引用/代码围栏/行内样式，iframe 沙箱无脚本）；html → 左编辑右实时预览分屏（iframe srcDoc，300ms 防抖，沙箱允许脚本）；图片/二进制走 FR5。
